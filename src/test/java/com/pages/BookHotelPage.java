...
public class BookHotelPage extends BaseClass {
	...
	public void bookHotel(String firstName, String lastName, String billingAddress, DataTable dataTable) {
		...
		elementSendkeys(getTxtCreditCardNumber(), data.get(1).get("creditCardNo"));
		selectOptionByText(getddnCreditCardType(), data.get(1).get("creditCardType"));
		selectOptionByText(getdDnMonth(), (data.get(1).get("expiryMonth")));
		selectOptionByText(getdDnYear(), (data.get(1).get("expiryYear")));
		elementSendkeys(getTxtCCVNumber(), (data.get(1).get("cVVNumber")));
